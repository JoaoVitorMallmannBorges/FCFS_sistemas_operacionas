    ...
    if opcao_2 == '1':
        ...
        while lista_copia !=[]:
            ...
            if disponivel == []:
                ...
                disponivel.sort(key=lambda p: (-p.prioridade, p.tempo_exec))
                processo = disponivel.pop(0)
                lista_copia.remove(processo)
                processo.tempo_restante -= 1
                print(f'tempo[{tempo}]: processo[{processo.posicao}] restante[{processo.tempo_restante}]')
        ...
        tempo = 0
        pedaco_tempo = int(input('Defina o time slice: '))
        lista_processos_copia = []
        lista_processos_copia = copy.deepcopy(lista_processos)
        while lista_processos_copia != []:
            processo = lista_processos_copia.pop(0)
            if processo.tempo_restante >= pedaco_tempo:
                for t in range(pedaco_tempo):
                    tempo += 1
                    processo.tempo_restante -= 1
                    print(f'tempo[{tempo}]: processo[{processo.posicao}] restante={processo.tempo_restante}')
                if processo.tempo_restante > 0:
                    lista_processos_copia.append(processo)
            else:
                for t in range(processo.tempo_restante):
                    tempo += 1
                    processo.tempo_restante -= 1
                    print(f'tempo[{tempo}]: processo[{processo.posicao}] restante={processo.tempo_restante}')
